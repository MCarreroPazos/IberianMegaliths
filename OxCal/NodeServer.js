...
function checkSettings()
{
 ...
 settings.home=path.resolve(settings.home);
 if(!settings.web)
 {
...
async function readDir(dir,ext,contains,within,detail,callback,errback)
{
  var rtn={"path":dir.replace(settings.home,"").replace(/\\/g,"/")+"/","files":[],"dirs":[]};
  var n,names,stat;
  try {
...
server=http.createServer(function (req, res) {
  ...
  function sendBack(err,txt)
  {
   ...
   return res.end(JSON.stringify({"status":true,"message":txt}));
  };
  // deal with specials 
  switch(q.pathname)
  {
  ...
  case "/setup.json":
   if(req.method=="POST")
   {
     ...
     req.on('end', function () {
       try
       {
        settings=qs.parse(body);
        checkSettings();
        settings.ok=true;
        console.log("Settings changed!");
        fs.writeFile(setupfile,JSON.stringify(settings), function(err) {
  ...
  if(q.query && q.query.action && (filename.indexOf('mydata')==0) && (filename.indexOf('..')==-1))
  {
   ...
   case "upload":
       ...
       try
       {
        ...
        case "save":
         fs.writeFile(filename,body, function(err) {
          if(err) { sendBack(err,"Save failed to "+q.pathname); }
          else{sendBack("","Write succeeded to "+q.pathname); }
         });
         break;
        case "upload":
         var enc,buf,mime;
         if(body.slice(0,5)!="data:"){throw new Error("Data in wrong format");};
         mime=body.slice(body.indexOf(":")+1,body.indexOf(";"));
         if(!mime){throw new Error("No mime format");}
         enc=body.slice(body.indexOf(";")+1,body.indexOf(","));
         if(!enc){throw new Error("No encoding");}
         buf = Buffer.from(body.slice(body.indexOf(",")+1),enc);
         fs.writeFile(filename,buf, function(err) {
          if(err) { sendBack(err,"Upload failed to "+q.pathname); }
          else{sendBack("","Write succeeded to "+q.pathname); }
         });
         break;
        };
    ...
    return;
   case "createDir":
    fs.mkdir(filename,{ recursive: true },err => {if(err){sendBack(err,"Failed to create "+q.pathname);}else{sendBack("","Created "+q.pathname)}});
    return;
   case "delete":
    try
    {
   ...
   case "readDir":
    readDir(filename,q.query.ext,q.query.contains,q.query.within,q.query.detail,function(rtn){
      ...
      res.writeHead(404, {'Content-Type': mimeType("json")});
      return res.end("404 Not Found")
    });
    break;
   };
