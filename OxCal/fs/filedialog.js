...
var OxCalURL="../oxcal/OxCal.html";
if(window.opener && (window.opener.oxcal || window.opener.runOxCalBinary))  // new version or old
{
 OxCalURL=window.opener.location.href.split(/[?#]/)[0];
};
...
function goGeneric(name)
{
 ...
 else
 {
  ...
  {
   path=setWorkingPathGeneric(name);
   setCookie("OxCalWorkingPath",path);
   refresh();
  };
 };
...
function fileUpload()
{
 if(onAServer())
 {
  refresh("action=upload&accept="+fileExt);
 }
 else
 {
  document.getElementById("lister").src="localUpload.html";
 };
};
function fileDownload()
{
 var fne=document.getElementById('FileName'),pom;
 if(fne && !filename)
 {
  filename=fne.value;
 };
 ...
 {
  fne=document.getElementById('PathName');
  if(fne && fne.value)
  {
   path=fne.value;
  };
  ...
  {
   pom.setAttribute('href', 'data:application/json;charset=utf-8,' + encodeURIComponent(/*'\ufeff'+*/filecontent));
  }
  else
  {
   pom.setAttribute('href',"..\/mydata"+path);
  };
...
function fileView()
{
 if(onAServer())
 {
  window.open(myDataUrl(getFullPath(filename,false)),"", "toolbar=yes,location=yes,menubar=no,resizable=yes,status=yes");
 ...
 {
  window.open(myDataUrl("../mydata"+getFullPath(filename,false)),"", "toolbar=yes,location=yes,menubar=no,resizable=yes,status=yes");
 };
};
function newFolder()
{
...
function doAction()
{
 ...
 {
 case "Insert":
  if(window.parent && window.parent.oxcal)
  {
   parent.app.fileReply(filename,"");
   break;
  };
  if(sourceWindow)
  {
 ...
 case "Run":
  ...
  if(sourceWindow && sourceWindow.runFile)
  {
 ...
 case "Open":
  ...
  {
  case "work":
    window.open(OxCalURL+"?view=status&source="+
     encodeURIComponent(getFullPath(filename.replace(".work",".js"),false)),"", "toolbar=yes,location=yes,menubar=yes,scrollbars=yes,resizable=yes,status=no");
    break;
  case "js":
   ...
   if((fileType=="All")&&(requestedAction==action))
   {
   ...
   {
    window.open(OxCalURL+"?source="+encodeURIComponent(getFullPath(filename,false)),"", "toolbar=yes,location=yes,menubar=yes,scrollbars=yes,resizable=yes,status=no");
   };
   break;
  case "oxcal":
   if(!parent.document.getElementById('fileDialog'))
   {
    window.open(OxCalURL+"?Mode=Input&source="+encodeURIComponent(getFullPath(filename,false)),"", "toolbar=yes,location=yes,menubar=yes,scrollbars=yes,resizable=yes,status=no");
    break;
   };
  case "json":
   if(parent.document.getElementById('fileDialog'))
...
function finishAction(force)
{
 if(getArgs().action||force)
 {
  if(parent.app)
  {
   ...
   {
    if(force===true){force="Cancelled by user";};
    if(force && parent && parent.app){parent.app.fileReply(false,force);};
    parent.app.hideTool("fileDialog");
   };
  };
  if(window.parent.document.getElementById('fileDialog'))
  {
