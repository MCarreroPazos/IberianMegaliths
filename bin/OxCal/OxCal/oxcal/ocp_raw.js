 ...
 {
  parent.oxcal.reportContent=document.getElementById(id).value;
  doSaveReported(ext);
 };
 function doSaveReported(ext)
 {
  parent.oxcal.reportFilename=ocd[plotOptions.plotFrom].name;
  parent.app.fileSaveAs("OxCalReport",ext).then(function(){}).catch(function(){});
 };
 function doSaveStr(str,ext)
 {
  parent.oxcal.reportContent=str;
  parent.oxcal.reportFilename=ocd[plotOptions.plotFrom].name;
  parent.app.fileSaveAs("OxCalReport",ext).then(function(){}).catch(function(){});
 };
  ...
  if(sep==',')
  {
   ...
   for(j=1;j<4;j++)
   {
    if(plotOptions.showRange[j])
    {
     switch(j)
     {
     case 1:str+='"from_68_3","to_68_3",';break;
  ...
  else
  {
   ...
   for(j=1;j<4;j++)
   {
    if(plotOptions.showRange[j])
    {
     switch(j)
     {
     case 1:str+='from_68_3'+sep+'to_68_3'+sep;break;
 ...
 function drawTable()
 {
  ...
  if(thisocd)
  {
   ...
   if(thisocd.order)
   {
    document.writeln("<input type='button' onclick='doSaveReported(\"csv\")' value='Save'\/>");
    ...
    case "Order":
     ...
     parent.oxcal.reportContent='"Order",';
	 break;
    case "Correl_Matrix":
     document.write("Pearson correlation coefficients");
     parent.oxcal.reportContent='"Correlation",';
	 break;
    case "Covar_Matrix":
     document.write("Covariance");
     parent.oxcal.reportContent='"Covariance",';
	 break;
    };
    ...
    for(i=0;i<thisocd.order.param.length;i++)
    {
     ...
     parent.oxcal.reportContent+='"'+thisocd.order.param[i]+'",';
    };
    parent.oxcal.reportContent+="\n";
    document.writeln("<\/tr>");
    for(j=0;j < thisocd.order.param.length;j++)
    {
     ...
     parent.oxcal.reportContent+='"'+thisocd.order.param[j]+'",';
     for(i=0;i<thisocd.order.param.length;i++)
     {
      document.write("<td>"+sigfig(thisocd.order.matrix[j][i],4)+"<\/td>");
      parent.oxcal.reportContent+=sigfig(thisocd.order.matrix[j][i],4)+',';
     };
     document.writeln("<\/tr>");
     parent.oxcal.reportContent+="\n";
    };
   ...
   else
   {
    ...
    if(left_col)
    {
     ...
     if(thisocd.likelihood&&thisocd.likelihood.comment)
     {
      ...
      {
       document.writeln(parent.oxcal.tidyText(thisocd.likelihood.comment[i]));
      };
     };
    ...
    if(left_col)
    {
     ...
     if(thisocd.posterior&&thisocd.posterior.comment)
     {
      ...
      {
       document.writeln(parent.oxcal.tidyText(thisocd.posterior.comment[i]));
      };
     };
     document.writeln("<\/pre><\/td>");
    };
 ...
 function parseProxyData(txt)
 {
  ...
  for(i=0;i<lines.length;i++)
  {
   ...
   if(((nos==0)&&(txts>1))||(i==0))
   {
    ...
    {
     headers[j]=headers[j].replace(/[\s\:\/\?\#\[\]\@\!\$\&\'\(\)\*\+\,\.\;\=\%\"\<\>\\\^\`\{\|\}\r\n]/g,"_");
     if(headers[j].search(/[0-9]/)==0){headers[j]="_"+headers[j];};
    };
   }
   else
   {
 ...
 function getDates(proxy)
 {
  ...
  function findGrads(l,k){var v; for(v=0;v<vs.length;v++){grad[vs[v]]=findGrad(vs[v],l,k);};};
  ...
  if(inta.length<1){parent.app.alert("No data!");return;};
  inta.sort(depthSort);
  ...
  if(l<=k){parent.app.alert("No model!");return;};
  findGrads(l,k);
  gerr=Math.sqrt(sqr(inta[l][dt_name])+sqr(inta[k][dt_name]))/(inta[l][z_name]-inta[k][z_name]);
  if(k!=0)
  {
   if(k>1){parent.app.alert("Interpolating to z="+inta[k][z_name]);};
   for(i=0;i<k;i++)
   {
    linearExtraps(i,k);
	inta[i][dt_name]=
  ...
  if(l!=inta.length-1)
  {
   parent.app.alert("Interpolating from z="+inta[l][z_name]);
 ...
 function doPlotProxy()
 {
  ...
  parent.oxcal.plotOnPlot(parseProxyData(document.getElementById("proxy_data").value),items,name,id,ts_id);
 };
 function doPlotDepth()
 {
  ...
  parent.oxcal.plotOnPlot(depthModelData,[],name,id,ts_id);
 };
 function putOnMap()
 {
  var i,j,id,name,dta;
  ...
  parent.oxcal.plotData=parent.app.clone(dta);
  parent.oxcal.mapPlot(false);
 };
 function initialise()
 {
